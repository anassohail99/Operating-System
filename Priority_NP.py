# Priority (Non Pre-emptive Approach) CPU Scheduling Algorithm
import numpy as np
import pandas as pd
import operator
...
for x in range(num_of_processes):
    Processes.append([x,'p'+str(x),x,int(input('Enter number of instructions for p'+str(x)+' = ')),0,int(input('Enter priority for p'+str(x)+' = '))])
    ...
    if(arrival_time<=len(Processes)-1):
        for x in range(len(Processes)):
            if(Processes[x][2]==arrival_time):
                ...
                RQ.sort(key=operator.itemgetter(5), reverse=True)
...
tabel = {
    ...
    'Priority':[x[5] for x in Processes],
